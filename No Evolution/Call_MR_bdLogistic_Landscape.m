...
% parameters for resource birth, death logistic growth

b = 4.5;                      params(1) = b; % prey birth rate

d = 1;                         params(2) = d; % prey death rate

qb = 8.75e-3;                      params(3) = qb; % birth density dependence

qd = 8.75e-3;                      params(4) = qd; % death density dependence

% K = b-d/(qb + qd)

% parameters for the interaction and predator

a = 0.12;                       params(5) = a; % space clearance rate

h = 0.41;                        params(6) = h; % handling time
...
for i = 1:size(prop_ext_highint_lowmag,1)
    ...
    i % just print which step the loop is on. 
...
% want to generate the minimum population densities in a different way that
% better reflects what is likely in the stochastic dynamics i.e. occurs
...
prop_ext_minpop = nan(length(a_vec), 4);

% store a and h values in the matrix

prop_ext_minpop(:,1) = h_vec;

prop_ext_minpop(:,2) = a_vec;

for i = 1:size(prop_ext_minpop,1)
    
    i % just print which step the loop is on.
    
   h = prop_ext_minpop(i,1);      params(6) = h; % get handling time
   
   a = prop_ext_minpop(i,2);      params(5) = a; % get space clearance rate
   ...
   % parameters for the interaction and predator
    ...
    % rerun the ode over the time span of the GEMs and with the new y0's 
    ...
    prop_ext_minpop(i,3) = min(y1(1:end, 1));
    
    prop_ext_minpop(i,4) = min(y1(1:end, 2));
    
end

table_prop_ext_minpop = array2table(prop_ext_minpop, 'VariableNames', {'HandlingTime', 'AttackRate', 'MinPrey', 'MinPred'});
