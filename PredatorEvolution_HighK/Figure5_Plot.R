...
ExtinctionHeatMap <- HeatMap_PopPersist2 + new_scale_color() + 
  ...
  geom_point(aes(x = 0.05, y = 0.0125), size = 3, color = 'white')   + 
...
AverageTrajectoryPlot <- ExtinctionHeatMap + new_scale_color() + 
 geom_line(data = filter(QG_data, Time < 36), aes(x =HandlingTime, y = SpaceClearanceRate), color = 'white', 
...
Trajectory_Zoomed <- ExtinctionHeatMap + new_scale_color() + geom_line(data = extant_ex_data, aes(x = HandlingTime, y = SCRate, group = Pop), color = 'green', alpha = 0.4) +
  ...
  xlim(c(0, 0.06)) + ylim(c(0,0.06)) +  geom_line(data = filter(QG_data, Time < 36), aes(x =HandlingTime, y = SpaceClearanceRate), color = 'white', 
                                                  arrow = arrow(length = unit(0.03, "npc"), ends = 'first'))
...
#### plots of variances in traits over time
...
save_plot(filename = 'SCRVariationPlot.png', SCRVarPlot)
...
### put the two plots together and save as a single file

VarPlots <- plot_grid(SCRVarPlot, HandlingVarPlot, nrow = 1, ncol = 2)

save_plot(filename = 'VarPlots_HighK.png', plot = VarPlots, nrow = 1, ncol = 2, bg = 'white') 
